...
# עיצוב CSS משודרג - ניווט בולט ומראה נקי
st.markdown("""
    <style>
    .main { direction: rtl; }
    h1, h2, h3, p, div, span { text-align: right; direction: rtl; font-family: 'Segoe UI', sans-serif; }
    
    /* העלמת תפריט הצד לחלוטין */
    [data-testid="stSidebar"] { display: none; }
    .block-container { padding-top: 2rem !important; }

    /* עיצוב הניווט העליון ככפתורי טאבים בולטים */
    ...
    /* התאמת תיבות בחירה וקלט למובייל */
...
# --- ניווט עליון בולט ---
# יצרנו כפתורי רדיו שנראים כמו טאבים בראש הדף
menu = st.radio("תפריט:", ["כניסת גמדים", "ניהול (HR)"], horizontal=True)
st.write("---")
...
if menu == "ניהול (HR)":
    ...
    if not st.session_state['admin_logged_in']:
        with st.form("admin_login"):
            pw = st.text_input("סיסמת מנהלת:", type="password")
    ...
    if data is not None and 'logged_in_user_id' not in st.session_state:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("כניסה למערכת"):
                if selected_name == "בחר/י שם...":
                    ...
                    if emp_id_input.strip() == str(user_row['ID']).strip():
                        st.session_state['logged_in_user_id'] = str(user_row['ID']).strip()
                        st.session_state['logged_in_name'] = selected_name
                        st.rerun()
                    else: st.error("מספר עובד לא תואם.")
        ...
        if try_val > 0:
            ...
            if button_placeholder.button("🎡 גלה מי הענק שלי!", key="play_btn"):
                ...
                data.at[user_idx, 'Timestamp'] = get_israel_time()
                conn.update(data=data)
                
                # אנימציית רולטה
                names = data['Name'].tolist()
                for i, t in zip(range(46), [0.06]*35 + [0.18]*8 + [0.5]*3):
                    result_placeholder.markdown(f"<h2 style='text-align: center; color: {'gray' if i < 35 else '#FF4B4B'};'>{random.choice(names)}</h2>", unsafe_allow_html=True)
                    time.sleep(t)
